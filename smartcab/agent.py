...
class LearningAgent(Agent):
    ...
    def build_state(self):
        ...
        state = (inputs.get('light'), inputs.get('oncoming'),
                 inputs.get('left'), inputs.get('right'), waypoint)
    ...
    def get_maxQ(self, state):
        ...
        max_action = max(self.Q[state].iteritems(), key=lambda kv: kv[1])[0]
        maxQ = dict[state][max_action]
        return maxQ
    ...
    def createQ(self, state):
        ...
        if state not in self.Q:
            self.Q[state] = dict(itertools.product(self.valid_actions, [0.0]))
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            max_value = max(self.Q[state].itervalues())
            actions = map(lambda sa: sa[0], filter(
                lambda kv: kv[1] == max_value, self.Q[state].iteritems()))
            if actions:
